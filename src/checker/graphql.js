...
function check2<SourceT, ArgsT, CxtT >(
  ...
  function graphqlResolver(
    ...
    let source = any_caster(_source);
    let args = any_caster(_args);
    let context = any_caster(context);
    try {
      if (sourceCaster) {
        source = sourceCaster(_source);
      }
    } catch (e) {
      const msg = `Source Check failed!\n${e}\n` +
        `With data:\n ${ePrint(_source)}`;
      throw new RunTimeCheckE(msg);
    }
    try {
      if (argsCaster) {
        args = argsCaster(_args);
      }
    } catch (e) {
      const msg = `Args Check failed!\n${e}\n` +
        `With data:\n ${ePrint(_args)}`;
      throw new RunTimeCheckE(msg);
    }
    try {
      if (contextCaster) {
        context = contextCaster(_context);
      }
    } catch (e) {
      const msg = `Context Check failed!\n${e}\n` +
        `With data:\n ${ePrint(context)}`;
      throw new RunTimeCheckE(msg);
    }
